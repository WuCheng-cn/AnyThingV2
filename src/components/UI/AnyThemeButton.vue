...
import PilotBear from '@/assets/svg/pilot-bear.svg'
...
<template>
  <span
    class="toggle inline-block h-4.5"
    :style="{
    ...
    <span class="toggle__content">
      ...
      <span aria-hidden="true" class="pilot__container">
        <span class="pilot-bear">
          <img :src="PilotBear" alt="pilot-bear" class="pilot">
        </span>
      </span>
      <svg
        aria-hidden="true"
...
.toggle__content {
  ...
  border-radius: var(--borderRadius);
  display: block;
  clip-path: inset( 0 0 0 0 round var(--borderRadius));
}
...
[data-pressed=false] .toggle__backdrop:last-of-type {
  transition-timing-function: cubic-bezier(.2,-0.6,.7,1.6);
}
[data-pressed=false] .stars path {
  transition-delay: 0s;
}
