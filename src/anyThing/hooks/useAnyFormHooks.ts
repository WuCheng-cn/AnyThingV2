...
export function useAnyFormHooks(props: IFormProps & {
  ...
  // 用于缓存选择字段的选项
  const selectFiledsOptions = ref<Record<string, IDictionary[]>>({})
  ...
  // 监听表单变化返回选项
  watchEffect(() => {
    const selectFileds = formFieldList.value.filter(field => getFieldConfig(field, 'formType') === EFormItemType.SELECT)
    selectFileds.forEach((field) => {
      const fn = getFieldConfig<'options'>(field, 'options')
      if (fn && typeof fn === 'function') {
        fn(formState.value).then((res) => {
          selectFiledsOptions.value[field] = res
        })
      }
    })
  })
  ...
  const getInputProps = computed(() => (field: string) => ({
    ...
    options: selectFiledsOptions.value[field] || formState.value.getOptions(field),
